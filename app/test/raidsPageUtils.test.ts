...
import { filterByDifficulty, filterByExtension } from "../../lib/raidUtils";
...
describe("filterByExtension", () => {
    it("retourne les raids si l'extension est 'Toutes'", () => {
        const result = filterByExtension(mockRaids, "Toutes");
        expect(result).toHaveLength(mockRaids.length);
    });

    it("filtre correctement par extension", () => {
        const result = filterByExtension(mockRaids, "Dawntrail");
        expect(result.every(raid => raid.extension === "Dawntrail")).toBe(true);
    });

    it("retourne un tableau vide si aucune raid ne correspond", () => {
        const result = filterByExtension(mockRaids, "NonExistentExtension");
        expect(result).toHaveLength(0);
    });
});